...
class Worker:
    """
    A class responsible for managing worker processes that interact with the environment.
    It generates episodes or evaluations based on assigned roles.
    ...
    def __init__(self, args: Dict[str, Any], conn: Any, wid: int):
        """
        Initializes the worker with environment, generator, and evaluator instances.
    ...
    def _gather_models(self, model_ids: List[int]) -> Dict[int, Optional[ModelWrapper]]:
        """
...
def make_worker_args(
    args: Dict[str, Any], n_ga: int, gaid: int, base_wid: int, wid: int, conn: Any
...
class Gather(QueueCommunicator):
    """
    A gather class responsible for handling multiple worker processes, buffering data,
    and communicating with a server for task assignment and result gathering.
    """

    def __init__(self, args: Dict[str, Any], conn: Any, gaid: int):
        """
        Initializes the gather process, opens worker connections, and sets up buffers.
...
class WorkerServer(QueueCommunicator):
    ...
    def run(self):
        """Start the server threads to accept connections and manage workers."""
...
def entry(worker_args: Dict[str, Any]) -> Dict[str, Any]:
    """Establishes a connection to the server and retrieves initial arguments."""
