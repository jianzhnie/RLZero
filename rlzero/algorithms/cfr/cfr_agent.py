...
class CFRAgent:
    ...
    def traverse_tree(self, probs, player_id):
        ...
        for action in legal_actions:
            ...
            self.average_policy[obs][action] += (self.iteration * player_prob *
                                                 action_prob)
        return state_utility
    ...
    def action_probs(self, obs, legal_actions, policy):
        ...
        if obs not in policy.keys():
            action_probs = np.array([
                1.0 / self.env.num_actions for _ in range(self.env.num_actions)
            ])
    ...
    def save(self):
        ...
        iteration_file = open(os.path.join(self.model_path, 'iteration.pkl'),
                              'wb')
