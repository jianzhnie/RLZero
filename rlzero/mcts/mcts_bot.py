...
class MCTSNode(ABC):
    ...
    def best_child(self, c_param=1.4):
        ...
        choices_weights = [
            (child_node.value / child_node.visit_count) + c_param * np.sqrt(
                (2 * np.log(self.visit_count) / child_node.visit_count))
...
class TwoPlayersMCTSNode(MCTSNode):
    ...
    def value(self):
        ...
        wins, loses = (
            self._results[1],
            self._results[-1]) if self.parent.env.current_player == 1 else (
...
class MCTS(object):
    ...
    def best_action(self,
        ...
        if best_action_type == 'UCB':
            ...
            self.root.best_action = self.root.expanded_actions[np.argmax(
                children_visit_counts)]
...
class MCTSBot:
    ...
    def get_actions(self, best_action_type='UCB'):
        """
        Overview:
