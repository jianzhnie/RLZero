...
class Evaluator(object):
    ...
    The evaluation function takes in an intermediate state in the game and returns an evaluation of that state, which
    should correlate with chances of winning the game. It returns the evaluation from all player's perspectives.
    """
...
class RandomRolloutEvaluator(Evaluator):
    ...
    This evaluator returns the average outcome of playing random actions from the given state until the end of the game.
        ...
        for _ in range(self.n_rollouts):
            ...
            while not working_state.is_terminal():
                if working_state.is_chance_node():
                    ...
                    action = self._random_state.choice(action_list,
                                                       p=prob_list)
                else:
                    action = self._random_state.choice(
                        working_state.legal_actions())
    ...
    def prior(self, state):
        ...
        if state.is_chance_node():
            ...
            return [(action, 1.0 / len(legal_actions))
    ...
    def to_str(self, state=None):
        ...
        return (
            ...
            'outcome: {}, {:3d} children').format(
                action, self.player, self.prior, self.explore_count
                and self.total_reward / self.explore_count, self.explore_count,
...
class MCTSBot(pyspiel.Bot):
    ...
    def _apply_tree_policy(self, root, state):
        ...
        working_state = state.clone()
        current_node = root
        while (not working_state.is_terminal() and current_node.explore_count >
               0) or (working_state.is_chance_node()
    ...
    def mcts_search(self, state):
        ...
        for _ in range(self.max_simulations):
            ...
            while visit_path:
                ...
                decision_node_idx = -1
                while visit_path[
                        decision_node_idx].player == pyspiel.PlayerId.CHANCE:
                    decision_node_idx -= 1
                ...
                if solved and node.children:
                    ...
                    if player == pyspiel.PlayerId.CHANCE:
                        ...
                        if (outcome is not None and all(
                                np.array_equal(c.outcome, outcome)
                        ...
                        for child in node.children:
                            if child.outcome is None:
                                all_solved = False
                            elif best is None or child.outcome[
                                    player] > best.outcome[player]:
                                best = child
                        if (best is not None and
                            (all_solved
                             or best.outcome[player] == self.max_utility)):
