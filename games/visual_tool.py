...
class VisualTool:
    ...
    def __init__(self, board_size=(8, 8), line_distance=75):
        ...
        self.canvas = Canvas(
            self.master,
            width=(self.board_size[0] + 1) * self.line_distance,
            height=(self.board_size[1] + 1) * self.line_distance + 70,
        )
    ...
    def graphic(self, x, y):
        ...
        if self.isblack:
            self.canvas.create_oval(
                x_location - (0.5 * self.line_distance) +
                0.1 * self.line_distance,
                y_location - (0.5 * self.line_distance) +
                0.1 * self.line_distance,
                x_location + (0.5 * self.line_distance) -
                0.1 * self.line_distance,
                y_location + (0.5 * self.line_distance) -
    ...
    def wininfo(self, winner):
        ...
        self.canvas.create_text(
            0.5 * (self.board_size[0] + 2) * self.line_distance,
            30,
    ...
    def onclick(self, event):
        if (self.isblack and self.can_click[0]) or (not self.isblack
                                                    and self.can_click[1]):
            # decide whether the click is valid
            if ((event.x > self.line_distance / 2)
                    and (event.x <
                         (self.board_size[1] + 0.5) * self.line_distance)
                    and (event.y > self.line_distance / 2)
                    and (event.y <
                         (self.board_size[0] + 0.5) * self.line_distance)):
                y = math.floor(
                    (event.x - self.line_distance / 2) / self.line_distance)
                x = (self.board_size[0] - 1 - (math.floor(
                    (event.y - self.line_distance / 2) / self.line_distance)))
    ...
    def draw(self):
        ...
        self.canvas.create_rectangle(
            0,
            0,
            (self.board_size[1] + 1) * self.line_distance,
            (self.board_size[0] + 1) * self.line_distance,
            fill='white',
            outline='white',
            tags=('chess_board'),
        )
        # create players info
        self.canvas.create_text(
            0.25 * (self.board_size[0] + 2) * self.line_distance,
            (self.board_size[0] + 1) * self.line_distance + 20,
            fill='darkblue',
            font='Times 20 italic bold',
            text='Black: ' + self.player_name[0],
        )
        self.canvas.create_text(
            0.65 * (self.board_size[0] + 2) * self.line_distance,
            (self.board_size[0] + 1) * self.line_distance + 20,
            fill='darkblue',
            font='Times 20 italic bold',
            text='White: ' + self.player_name[1],
        )
        # create board
        for x in range(0, (self.board_size[1] + 1) * self.line_distance,
                       self.line_distance):
            self.canvas.create_line(
                x,
                self.line_distance,
                x,
                self.board_size[0] * self.line_distance,
                fill='#476042',
            )
        for y in range(0, (self.board_size[0] + 1) * self.line_distance,
                       self.line_distance):
            self.canvas.create_line(
                self.line_distance,
                y,
                self.board_size[1] * self.line_distance,
                y,
                fill='#476042',
            )
