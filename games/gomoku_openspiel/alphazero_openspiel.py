...
def _init_model_from_config(config):
    return model_lib.Model.build_model(config.nn_model,
                                       config.observation_shape,
                                       config.output_size, config.nn_width,
                                       config.nn_depth, config.weight_decay,
                                       config.learning_rate, config.path)
...
def actor(*, config, game, logger, queue):
    ...
    for game_num in itertools.count():
        if not update_checkpoint(logger, queue, model, az_evaluator):
            return
        queue.put(
            _play_game(logger, game_num, game, bots, config.temperature,
...
def learner(*, game, config, actors, evaluators, broadcast_fn, logger):
    ...
    def learn(step):
        ...
        save_path = model.save_checkpoint(step if step %
                                          config.checkpoint_freq == 0 else -1)
...
def alpha_zero(config: Config):
    ...
    config = config._replace(observation_shape=game.observation_tensor_shape(),
